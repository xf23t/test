...
router.post('/', function (req, res, next) { //닉네임 중복확인 
    ...
    connection.query('select count(*) cnt from user where user_nick=?;', [nick], function (error, cursor) {
        ...
        if (cnt == 0) {
            ...
            connection.query('update user set user_nick = ? where user_idx= ? ;', [nick, idx], function (err, info) {
                if (err == null) {

                    connection.query('select * from user where user_idx=?;', [idx], function (er, cur) {

                        if (cur.length > 0) {

                            res.json({
                                result: true,
                                nick: cur[0].user_nick,
